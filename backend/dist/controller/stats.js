...
export const getDashBoardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has('admin-stats'))
        ...
        const [thisMonthProducts, thisMonthUsers, thisMonthOrders, lastMonthProducts, lastMonthUsers, lastMonthOrders, productCount, userCount, orderRevenue, lastSixMonthOrders, categories, femaleCount, latestTransactions] = await Promise.all([
        ...
        const revenue = orderRevenue.reduce((total, order) => total + (order.total || 0), 0);
        const counts = {
            user: userCount,
...
export const getDashBoardPie = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has('admin-pie-charts')) {
        ...
        const [processingOrder, shippedOrder, deliveredOrder, categories, outOfStock, inStock, users, adminCount, allOrders] = await Promise.all([
...
export const getDashBoardLine = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        lines = JSON.parse(myCache.get(key));
    else {
        const today = new Date();
        const twelveMonthsAgo = new Date();
        twelveMonthsAgo.setMonth(twelveMonthsAgo.getMonth() - 12);
