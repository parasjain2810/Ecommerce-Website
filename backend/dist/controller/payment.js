...
export const applyDiscount = TryCatch(async (req, res, next) => {
    ...
    if (!discount)
        return next(new ErrorHandler("NO coupon found", 300));
...
export const allCoupons = TryCatch(async (req, res, next) => {
    ...
    if (!AllCoupons)
        return next(new ErrorHandler("No Coupons are found", 701));
    return res.status(201).json({
        success: true,
        AllCoupons
    });
...
export const DeleteCoupon = TryCatch(async (req, res, next) => {
    ...
    if (!coupon)
        return next(new ErrorHandler("No Coupon is found", 701));
