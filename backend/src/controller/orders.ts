...
export const getOrders=TryCatch(async(req,res,next)=>{
    ...
    if(myCache.has('getOrders')){
      order=JSON.parse(myCache.get('getOrders') as string);
    }
    else{
...
export const myOrder=TryCatch(async(req,res,next)=>{
  ...
  if(myCache.has(`myorder-${user}`)){
    order=JSON.parse(myCache.get(`myorder-${user}`)as string);
  }
  else{
      order=await Order.find({user:user}).populate("user","name");
      if(!order) return next(new ErrorHandler('No order Id found',301))
...
export const getSingleOrder=TryCatch(async(req,res,next)=>{
  ...
  if(myCache.has(`single-order${id}`)){
    ...
    if(!order) return next(new ErrorHandler('No order found',501));
...
export const processOrder=TryCatch(async(req,res,next)=>{
  ...
  if(!order) return next(new ErrorHandler("No Id found",501))
...
export const deleteOrder=TryCatch(async(req,res,next)=>{
  ...
  if(!order) return next(new ErrorHandler("No id Found",506));
